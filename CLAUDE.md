...
`public/companion.ps1` — a PowerShell 5.1 tray app, installed via `irm https://coachbuild.vercel.app/companion.ps1 | iex -Install`, versioned independently of the web app (currently 1.7.0; served over `irm|iex` so users must re-run the install one-liner to pick up a bump — check each CHANGELOG entry's "(COMPANION CHANGE → x.y.z — re-install required)" tag). It watches the League client via the LCU API and bridges to the browser over a local HTTP server (127.0.0.1-only, exact-Origin CORS, per-launch or persisted session token).
