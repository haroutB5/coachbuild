...
import { deltaVsLaneAverage, formatDeltaPoints } from "./draft/draftDelta";
            ...
            {rankingRows.map(({ candidate, rank }) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={`px-2 py-2.5 text-right text-[11px] font-semibold tabular-nums ${delta === null ? "text-txt/[0.38]" : delta >= 0 ? "text-good" : "text-bad"}`}>{formatDeltaPoints(delta)}</td>
            ...
            return (
              ...
              >
                ...
                <div className="col-start-2 col-span-2 mt-2 grid grid-cols-4 gap-2 border-t border-txt/[0.06] pt-2 text-right">
                  ...
                  <div><span className="block text-[8px] uppercase tracking-[0.1em] text-txt/[0.35]">Delta lane avg</span><span className={`mt-0.5 block text-[11px] font-semibold tabular-nums ${delta === null ? "text-txt/[0.38]" : delta >= 0 ? "text-good" : "text-bad"}`}>{formatDeltaPoints(delta)}</span></div>
