...
describe("situational — the WPA deltas the desktop overlay draws (0.114.0 wire)", () => {
  ...
  it("buildItemSets derives the row ONCE and hands it to both consumers (source)", () => {
    ...
    // The block and the wire must both take `picks`, the ONE already-filtered
    // array. The WPA-build exclusion is what makes them different LENGTHS from
    // the raw shortlist, so a wire rebuilt independently paints each delta over
    // the wrong icon while agreeing on nothing a size check would catch.
    ...
    // 0.120.0: the comp no longer reorders this row at all -- it gets its own
    // `For this game` block instead -- so neither call site may take a
    // promotion argument again without this test going red.
    expect(body).not.toMatch(/situationalBlockPicks\(items,\s*wpaBuildIds,/);
    expect(body).not.toMatch(/situationalBlocks\(picks,/);
  });
