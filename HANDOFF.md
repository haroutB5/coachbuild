...
## Latest dispatch -- 2026-07-21 20:43
...
## Latest dispatch -- 2026-07-22 15:29
...
## Latest dispatch -- 2026-07-22 17:20
...
## Latest dispatch -- 2026-07-22 17:59
...
<!-- merged into HANDOFF.md 2026-07-22 16:20:06Z; previous content preserved there. Append new rounds below. -->

## 2026-07-22 — v0.48.3: pro-rune SLOT COHERENCE (WEB-ONLY, no companion change)

**Reported:** in-client "CoachBuild Ashe Bot Pro" applied with EMPTY primary-tree minor slots (only keystone Lethal Tempo selected). The `…Pro` page itself is correct (1.6.3); this is about page CONTENTS.

**Confirmed root cause (probed against live prod feed, not just the briefed hypothesis):**
- `proConsensusRuneApplyInput` built `runes.primary` from `model.primaryMinors.entries` — a FLAT top-3-by-FREQUENCY list, NO per-row structure. A valid LCU page needs exactly one rune per perkstyles row (0/1/2). Flat frequency can rank two runes from the SAME row above a third row's rune (or emit a tree-foreign rune) → the client has no valid perk for the missing row → EMPTY minor slot. `missingRunePageReason`'s `entries.length < 3` count check passed it (3 total = 2-from-one-row + empty row).
- The WPA "Apply runes" button never had this — `lib/recommend.ts`'s `rowPicks` selects one rune PER ROW. The pro path bypassed that guarantee.
- **Probe finding that corrected the brief:** the brief assumed "the app already fetches perkstyles for slot validity." It does NOT — slot structure existed ONLY as a fixture in `runeApplyBody.test.ts`. Built a real runtime slot map. Also: Ashe bot is **97% prostage** (Leaguepedia), whose `primary[]` is NOT row-ordered — so index/positional slotting alone is insufficient; ids must be resolved by the perkstyles MAP. (soloq `primary[]` IS row-ordered, so it's read positionally — this is what fills all 3 rows from a single real game.)

**Fix (slot-coherent by construction):**
- NEW `components/hextech/perkSlots.ts` — id→(tree,row) map for all 5 trees + keystones, pinned from a live CDragon `perkstyles.json` pull (2026-07-22). Helpers `primaryMinorRow`, `isKeystoneOf`. Drift is graceful (unknown id → row null → filled from another game or honest disable, never a wrong slot).
- `aggregateProConsensus` emits additive `runePage: SlotCoherentRunePage` — per-row modal over the conditioned page sample (soloq positional, prostage via map). `primaryRows[3]`/`secondaryRows[3]`, null = uncoverable row.
- `proConsensusRuneApplyInput` reads `runePage`: primary in ROW order (0→1→2), 2 most-adopted secondary rows in ascending row order. `missingRunePageReason` checks the per-ROW structure (slot gap), not a raw count. The FLAT `primaryMinors`/`secondaryPicks` stay the card DISPLAY source — untouched (deliberate scope boundary; display coherence is a pre-existing lesser issue, noted as possible follow-up).

**Ashe-bot before/after (real prod 100-game feed, run through the real code path):**
- BEFORE (flat, count-desc order): primary emitted `[8017(row2), 8009(row0), 9103(row1)]` — scrambled row order; and on a colliding/thin sample, two same-row ids + a missing row.
- AFTER: `selectedPerkIds = [8008(Lethal Tempo), 8009(row0), 9103(row1), 8017(row2), 8313(Inspiration secRow1), 8410(secRow2), 5008, 5010, 5011]`, primaryStyleId 8000 / subStyleId 8300, name "CoachBuild Ashe Bot Pro". Keystone valid, one minor per row in row order, 2 secondary picks from 2 distinct rows. A complete legal page.

**Thin-data fill decision:** per-row modal already realizes "fill from real games" — cross-game at the ROW level (game A covers rows 0/1, game B covers row 2 → complete), and a single complete soloq game fills all 3 rows positionally. So the button stays usable on any sample containing ≥1 complete real page. A row absent from EVERY sampled game → button DISABLED with reason (no "modal game" could supply it either) — never fabricate/empty a slot.

**Tests:** extended `proConsensus.test.ts` — complete/thin-1-game/cross-game-fill (usable) vs. uncoverable-slot + same-row-collision (disabled), all perkstyles-validated; fixed pre-existing fixtures that used slot-INCOHERENT triplets (`[MANAFLOW_BAND(r0), CELERITY(r1), TRANSCENDENCE(r1)]` — two row1s, no row2) which the old flat logic silently accepted. `verify-fix.sh` ALL GREEN, 1422 tests.

**On device the user should see:** applying the Pro page now fills the keystone + ALL 3 primary minor rows + both secondary picks. WEB-ONLY — no companion re-install.
...
## Latest dispatch -- 2026-07-24 15:07
...
## Latest dispatch -- 2026-07-24 15:43
