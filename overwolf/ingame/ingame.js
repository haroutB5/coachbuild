...
// is what the background controller drives, but it cannot call these functions
// directly: Overwolf windows are separate browser contexts, and there is no
// supported API for a background page to obtain a live JS reference into another
// window's global scope. `overwolf.windows.sendMessage` / `onMessageReceived` is
// the documented cross-window transport, so the calls happen HERE, in response to
// a received message. The message ids are the exact values of background.js's
// MESSAGES constant -- if either side is renamed, both must change together.
const MESSAGE_IDS = {
...
if (typeof overwolf !== "undefined" && overwolf.windows && overwolf.windows.onMessageReceived) {
  overwolf.windows.onMessageReceived.addListener((message) => {
    if (!message) return;
    // Never trust the payload shape: this crosses a process-ish boundary and the
    // sender is a different window that can be reloaded independently of us.
    try {
      if (message.id === MESSAGE_IDS.STATE) {
        window.CoachBuildOverlay.onState(message.content);
      } else if (message.id === MESSAGE_IDS.INTERACTIVE) {
      ...
      console.warn("[CoachBuild overlay] failed to apply pushed message:", err);
    }
  });

  // Announce readiness. `sendMessage` is fire-and-forget with no queue, so a
  // state push sent before this file finished loading was DROPPED, not buffered
  // -- and since this window only ever receives pushes and can never pull, that
  // drop would leave the overlay blank until the next GEP change (a level-up,
  // i.e. potentially minutes) at exactly the moment the player most wants it.
  // The background controller answers this with a fresh snapshot.
  overwolf.windows.sendMessage("background", MESSAGE_IDS.READY, {}, () => {
    // Deliberately empty: if the background window is not up yet, there is
    // nothing useful to do and nothing worth alarming the user about. It will
    // push state on its own once GEP produces some.
  });
}
